...
login.addEventListener('input', function() {
    if (login.value !== login.value.replace(/[^а-яА-Яa-zA-Z0-9-_]/g, characterСheck(login))){ // проверка что в логине нет недопустимых символов
        errorPattern(login, `Недопустимый символ - ${inputСharacter}`);
